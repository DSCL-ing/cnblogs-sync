...
有序才能建立索引->因此mysql插入时内部会默认以主键进行排序

为什么非叶子节点不存数据? 因为能存更多的目录项,即一个目录页能够管理更多的页表项;-> 使树的形状: 矮胖(特别高效) -> 索引时途径的节点更少 -> 找到目标数据更快 == 在实际与数据比较时,IO次数减少 -> IO层面提高了效率



MySQL 存储引擎InnoDB 的索引结构: B+树
...
是的,MySQL在建表时,会有默认的主键(自动生成一个隐藏列),如果我们不显示定义主键,则使用默认的;

隐藏列的顺序大小就是插入顺序;



没有被构建索引的列,查询时只能是线性遍历;



所谓构建索引,就是构建一棵特定key的B+树;

如果没有构建索引,MySQL就只能以线性遍历方式进行查找;



一个索引就是一棵B+树,多张表就有多棵B+树,因此多张表存在MySQL内就会存在多棵B+树;

B+树只是逻辑上完整存在,实际MySQL的空间中,加载出来的索引树都是残缺的,是根据索引的结果加载一部分(所在子树:根节点和孩子;确认孩子后,继续递归,直到找到数据项)



B+树的特点:B+树的叶子节点全部是串链起来的;

> 索引树的叶子节点也是全部串链起来 --> 支持小范围查找,只需要确定起点和终点,然后线性遍历
...
读 未提交: 脏读: 一个事务还未提交.但增删改行为能被并发的另一个事务读取到,这样读到的数据是脏的,即脏读.

读 提交: 不可重复读: 一个事务提交后,并发的另一个事务执行过程中重复读可能得到的结果是不一样的,因此不可重复读.
